...
import { z } from 'zod';
import {
  MAX_REFACTOR_OPERATIONS,
  runRefactorArchitecture,
} from './refactor.js';
...
export function registerRefactorTools(
  server: McpServer,
  resolveProjectId: (provided?: string) => string
): void {
  server.registerTool(
    ...
    async ({
      ...
      const result = await runRefactorArchitecture(projectId, {
        operations: operations as Parameters<typeof runRefactorArchitecture>[1]['operations'],
        scope,
        dryRun,
        confirm,
        limit,
