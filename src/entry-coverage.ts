...
function entriesForModule(entries: Entry[], moduleName: string): Entry[] {
  return entries.filter((e) => e.refs?.moduleName === moduleName);
}

function hasKind(entries: Entry[], kinds: Set<string>): boolean {
...
export function validateEntryCoverage(
  ...
  if (architecture) {
    ...
    for (const mod of architecture.modules) {
      ...
      if (
        files.some((f) => fileMatches(f, /Repository/i) || fileMatches(f, /Entity/i)) &&
