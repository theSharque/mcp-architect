...
function edgeSet(flow: DataFlow | undefined): Set<string> {
...
export function recomputeProvidesTo(dataFlow: DataFlow, moduleNames: string[]): DataFlow {
...
export function syncModuleDependsOn(
  dataFlow: DataFlow,
  moduleName: string,
  dependsOn: string[],
  moduleNames: string[]
): DataFlow {
  const updated: DataFlow = { ...dataFlow };
  updated[moduleName] = {
    ...(updated[moduleName] ?? {}),
...
export function buildDataFlowFromModules(
...
export function buildDataFlowFromDependsOn(dataFlow: DataFlow, moduleNames: string[]): DataFlow {
  const flow: DataFlow = {};
  for (const name of moduleNames) {
...
export function pruneDataFlow(dataFlow: DataFlow, validNames: string[]): DataFlow {
  const names = new Set(validNames);
  const pruned: DataFlow = {};
...
export function removeModuleFromDataFlow(
...
export function mergeModules(
  existing: ModuleSummary[] | undefined,
  incoming: ModuleSummary[],
...
export function mergeDataFlow(
  existing: DataFlow | undefined,
  incoming: DataFlow | undefined,
  options: { replace: boolean }
): DataFlow | undefined {
  if (options.replace) {
    return incoming;
  ...
  const merged: DataFlow = { ...existing };
...
export function validateDataFlow(
  ...
  moduleDetailsMap: Map<string, ModuleDetails>,
  options: { checkInverse?: boolean; checkModuleDeps?: boolean } = {}
): ValidationIssue[] {
...
export function diffFlowEdges(
  before: DataFlow | undefined,
  after: DataFlow | undefined
