...
export type RefactorOperation =
  | { type: 'scan'; file?: string; text?: string }
  | { type: 'move-file'; from: string; to: string }
  | { type: 'replace-path-prefix'; fromPrefix: string; toPrefix: string }
  | {
      type: 'rename-text';
      from: string;
      to: string;
  ...
  | { type: 'patch-entry'; match: RefactorPatchMatch; set: RefactorPatchSet }
  | { type: 'merge-files'; from: string[]; to: string }
  | { type: 'remove-file-ref'; file: string; deleteIfEmpty?: boolean };
