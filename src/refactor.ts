...
function cloneModule(module: ModuleDetails): ModuleDetails {
  return structuredClone(module);
}

function scopeToReplaceScope(scope?: RefactorScope): ReplaceEntriesScope {
  return {
...
function entryInScope(entry: Entry, scope?: RefactorScope): boolean {
  if (!scope) {
    return true;
  }
...
function moduleInScope(module: ModuleDetails, scope?: RefactorScope): boolean {
  if (!scope?.moduleName) {
    return true;
  }
...
function replaceInPayload(
  ...
  const next = walk(payload) as Record<string, unknown>;
  return { next: changed ? next : payload, changed };
}

function replaceInUsageExamples(
  examples: UsageExample[] | undefined,
  from: string,
  to: string,
  match: RefactorTextMatch
): { next: UsageExample[] | undefined; changed: boolean } {
  if (!examples?.length) {
  ...
  const next = examples.map((example) => {
    const updated: UsageExample = { ...example };
    for (const key of ['title', 'description', 'command', 'input', 'output', 'notes'] as const) {
...
function replaceFilePath(path: string, from: string, to: string, exact: boolean): string | null {
  if (exact) {
...
function replaceFilesList(
  files: string[] | undefined,
  replacer: (file: string) => string | null
...
function shouldDeleteOrphan(entry: Entry): boolean {
...
function findEntryByMatch(
  state: RefactorDraftState,
...
function recordEntryDelete(changes: RefactorChange[], entry: Entry): void {
  changes.push({
    action: 'delete',
    target: 'entry',
    id: entry.id,
...
export function assertRefactorOperationLimit(count: number): void {
  if (count > MAX_REFACTOR_OPERATIONS) {
    throw new Error(
      `Too many refactor operations: max ${MAX_REFACTOR_OPERATIONS} per call`
    );
  }
}

export function clampRefactorLimit(limit?: number): number {
...
export function clampRefactorOffset(offset?: number): number {
...
function applyMoveFile(
  ...
  for (const entry of state.entries.values()) {
    ...
    const { next, changed } = replaceFilesList(entry.refs?.files, (file) =>
      replaceFilePath(file, from, to, true)
    );
    if (!changed || !next) {
      continue;
    }
  ...
  for (const module of state.modules.values()) {
    ...
    const { next, changed } = replaceFilesList(module.files, (file) =>
      replaceFilePath(file, from, to, true)
    );
    if (!changed || !next) {
      continue;
...
function applyReplacePathPrefix(
  state: RefactorDraftState,
  fromPrefix: string,
  toPrefix: string,
  scope: RefactorScope | undefined,
  ...
  for (const entry of state.entries.values()) {
    ...
    const before = entry.refs?.files ? [...entry.refs.files] : undefined;
    const { next, changed } = replaceFilesList(entry.refs?.files, (file) =>
      replaceFilePath(file, fromPrefix, toPrefix, false)
    );
    if (!changed || !next) {
      continue;
    }
...
function applyRenameTextToEntry(
  state: RefactorDraftState,
  entry: Entry,
  from: string,
  to: string,
  ...
  for (const field of fields) {
    if (field === 'title') {
      ...
      if (hasTitleCollision(state, entry.id, entry.kind, replaced)) {
        pushWarning(
          warnings,
          `Skipped title change for entry ${entry.id}: '${entry.kind}' / '${replaced}' already exists`
        );
        continue;
      }
...
function applyPatchEntry(
  ...
  if (set.title !== undefined && set.title !== entry.title) {
    if (hasTitleCollision(state, entry.id, entry.kind, set.title)) {
      pushWarning(
        warnings,
        `Skipped patch title for entry ${entry.id}: '${entry.kind}' / '${set.title}' already exists`
      );
    } else {
      const before = entry.title;
...
function computeStats(
  ...
  originalModules: Map<string, ModuleDetails>,
  state: RefactorDraftState,
  deletedIds: string[]
): RefactorStats {
...
export function executeRefactorDraft(
  state: RefactorDraftState,
  ...
  const originalEntries = new Map(
    [...state.entries.entries()].map(([id, entry]) => [id, cloneEntry(entry)])
  );
  const originalModules = new Map(
    [...state.modules.entries()].map(([id, module]) => [id, cloneModule(module)])
  );
  ...
  for (const operation of request.operations) {
    switch (operation.type) {
      ...
      case 'replace-path-prefix':
        applyReplacePathPrefix(
          state,
          operation.fromPrefix,
          operation.toPrefix,
          request.scope,
          changes
        );
...
export function paginateRefactorResult(
  dryRun: boolean,
  hits: RefactorScanHit[] | undefined,
  changes: RefactorChange[],
  warnings: string[],
  stats: RefactorStats,
  limit?: number,
...
export async function runRefactorArchitecture(
  projectId: string,
  request: RefactorRequest
